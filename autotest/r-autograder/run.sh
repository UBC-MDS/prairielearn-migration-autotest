...
SERVICE_DIR="${JOB_DIR}/serverFilesCourse/autograder"
...
cp    ${VFLAG}  ${SERVICE_DIR}/*  ${MERGE_DIR}  ## added this line
...
if [ -d "${TEST_DIR}" ]; then
    ...
    echo '{"gradable": false, "score": 0.0, "message": "Grading error! Contact course staff and have them check the logs for this submission."}' > results.json
fi
